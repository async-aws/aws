...
final class PublishInput extends Input
{
     ...
     * Constraints:
     *
     * - With the exception of SMS, messages must be UTF-8 encoded strings. By default, a message can be at most 256 KiB in
     *   size (262,144 bytes, not 262,144 characters).
     *
     *   When you publish to a topic, the maximum size is determined by the topic's `MaximumMessageSize` attribute, which
     *   supports values up to 1 MiB (1,048,576 bytes). Amazon SNS validates the combined size of the message body and
     *   message attributes against this value and returns an `InvalidParameter` error if the limit is exceeded.
     *
