...
before uploading. You want to do it in a smarter way. AsyncAws allows you to upload
files using a string, resource, closure, or an iterable. See the following examples:
...
content is not a file (i.e. streamed from an HTTP request). But some third-party
services like OpenStack Swift, which present themselves as "S3-compatible," do not
