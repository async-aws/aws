...
class S3Client extends AbstractApi
{
    ...
    public function putObject(array $input): PutObjectOutput
    {
        ...
        if (array_key_exists("CacheControl", $input)) $headers["Cache-Control"] = $input["CacheControl"];
        if (array_key_exists("ContentDisposition", $input)) $headers["Content-Disposition"] = $input["ContentDisposition"];
        if (array_key_exists("ContentEncoding", $input)) $headers["Content-Encoding"] = $input["ContentEncoding"];
        if (array_key_exists("ContentLanguage", $input)) $headers["Content-Language"] = $input["ContentLanguage"];
        if (array_key_exists("ContentLength", $input)) $headers["Content-Length"] = $input["ContentLength"];
        if (array_key_exists("ContentMD5", $input)) $headers["Content-MD5"] = $input["ContentMD5"];
        if (array_key_exists("ContentType", $input)) $headers["Content-Type"] = $input["ContentType"];
        if (array_key_exists("Expires", $input)) $headers["Expires"] = $input["Expires"];
        ...
        if (array_key_exists("ServerSideEncryption", $input)) $headers["x-amz-server-side-encryption"] = $input["ServerSideEncryption"];
        if (array_key_exists("StorageClass", $input)) $headers["x-amz-storage-class"] = $input["StorageClass"];
        if (array_key_exists("WebsiteRedirectLocation", $input)) $headers["x-amz-website-redirect-location"] = $input["WebsiteRedirectLocation"];
        if (array_key_exists("SSECustomerAlgorithm", $input)) $headers["x-amz-server-side-encryption-customer-algorithm"] = $input["SSECustomerAlgorithm"];
        if (array_key_exists("SSECustomerKey", $input)) $headers["x-amz-server-side-encryption-customer-key"] = $input["SSECustomerKey"];
        if (array_key_exists("SSECustomerKeyMD5", $input)) $headers["x-amz-server-side-encryption-customer-key-MD5"] = $input["SSECustomerKeyMD5"];
        if (array_key_exists("SSEKMSKeyId", $input)) $headers["x-amz-server-side-encryption-aws-kms-key-id"] = $input["SSEKMSKeyId"];
        if (array_key_exists("SSEKMSEncryptionContext", $input)) $headers["x-amz-server-side-encryption-context"] = $input["SSEKMSEncryptionContext"];
        if (array_key_exists("RequestPayer", $input)) $headers["x-amz-request-payer"] = $input["RequestPayer"];
        if (array_key_exists("Tagging", $input)) $headers["x-amz-tagging"] = $input["Tagging"];
        if (array_key_exists("ObjectLockMode", $input)) $headers["x-amz-object-lock-mode"] = $input["ObjectLockMode"];
        if (array_key_exists("ObjectLockRetainUntilDate", $input)) $headers["x-amz-object-lock-retain-until-date"] = $input["ObjectLockRetainUntilDate"];
        if (array_key_exists("ObjectLockLegalHoldStatus", $input)) $headers["x-amz-object-lock-legal-hold"] = $input["ObjectLockLegalHoldStatus"];
