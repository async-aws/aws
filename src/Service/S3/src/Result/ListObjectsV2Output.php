...
class ListObjectsV2Output extends Result implements \IteratorAggregate
{
    ...
    private function populateResultChecksumAlgorithmList(\SimpleXMLElement $xml): array
    {
        $items = [];
        foreach ($xml as $item) {
            $items[] = !ChecksumAlgorithm::exists((string) $item) ? ChecksumAlgorithm::UNKNOWN_TO_SDK : (string) $item;
        }
    ...
    private function populateResultObject(\SimpleXMLElement $xml): AwsObject
    {
        return new AwsObject([
            ...
            'StorageClass' => (null !== $v = $xml->StorageClass[0]) ? (!ObjectStorageClass::exists((string) $xml->StorageClass) ? ObjectStorageClass::UNKNOWN_TO_SDK : (string) $xml->StorageClass) : null,
