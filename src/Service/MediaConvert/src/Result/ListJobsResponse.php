...
/**
 * Successful list jobs requests return a JSON array of jobs. If you don't specify how they are ordered, you will
 * receive the most recently created first.
 *
 * @implements \IteratorAggregate<Job>
 */
class ListJobsResponse extends Result implements \IteratorAggregate
{
    /**
     * List of jobs.
     *
     ...
     * @return iterable<Job>
     */
    public function getJobs(bool $currentPageOnly = false): iterable
    {
        if ($currentPageOnly) {
            $this->initialize();
        ...
        if (!$this->input instanceof ListJobsRequest) {
    ...
    protected function populateResult(Response $response): void
    {
        ...
        $this->jobs = empty($data['jobs']) ? [] : $this->populateResult__listOfJob($data['jobs']);
    ...
    private function populateResult__listOfAudioChannelTag(array $json): array
    {
        $items = [];
        foreach ($json as $item) {
            $a = isset($item) ? (!AudioChannelTag::exists((string) $item) ? AudioChannelTag::UNKNOWN_TO_SDK : (string) $item) : null;
            if (null !== $a) {
                $items[] = $a;
            }
        }
    ...
    private function populateResult__listOfFrameMetricType(array $json): array
    {
        $items = [];
        foreach ($json as $item) {
            $a = isset($item) ? (!FrameMetricType::exists((string) $item) ? FrameMetricType::UNKNOWN_TO_SDK : (string) $item) : null;
            if (null !== $a) {
                $items[] = $a;
            }
        }
    ...
    private function populateResult__listOfHlsAdMarkers(array $json): array
    {
        $items = [];
        foreach ($json as $item) {
            $a = isset($item) ? (!HlsAdMarkers::exists((string) $item) ? HlsAdMarkers::UNKNOWN_TO_SDK : (string) $item) : null;
            if (null !== $a) {
                $items[] = $a;
            }
        }
     ...
     * @return Job[]
     */
    private function populateResult__listOfJob(array $json): array
    {
        $items = [];
        foreach ($json as $item) {
            $items[] = $this->populateResultJob($item);
        }
    ...
    private function populateResult__listOfTeletextPageType(array $json): array
    {
        $items = [];
        foreach ($json as $item) {
            $a = isset($item) ? (!TeletextPageType::exists((string) $item) ? TeletextPageType::UNKNOWN_TO_SDK : (string) $item) : null;
            if (null !== $a) {
                $items[] = $a;
            }
        }
