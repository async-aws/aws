...
class IndexFacesResponse extends Result
{
    ...
    protected function populateResult(Response $response): void
    {
        ...
        $this->FaceRecords = empty($data['FaceRecords']) ? [] : (function (array $json): array {
            $items = [];
            foreach ($json as $item) {
                $items[] = new FaceRecord([
                    ...
                    'FaceDetail' => empty($item['FaceDetail']) ? null : new FaceDetail([
                        ...
                        'Emotions' => empty($item['FaceDetail']['Emotions']) ? [] : (function (array $json): array {
                            $items = [];
                            foreach ($json as $item) {
                                $items[] = new Emotion([
                            ...
                            return $items;
                        })($item['FaceDetail']['Emotions']),
                        'Landmarks' => empty($item['FaceDetail']['Landmarks']) ? [] : (function (array $json): array {
                            $items = [];
                            foreach ($json as $item) {
                                $items[] = new Landmark([
                                    'Type' => isset($item['Type']) ? (string) $item['Type'] : null,
        ...
        $this->UnindexedFaces = empty($data['UnindexedFaces']) ? [] : (function (array $json): array {
            $items = [];
            foreach ($json as $item) {
                $items[] = new UnindexedFace([
                    'Reasons' => empty($item['Reasons']) ? [] : (function (array $json): array {
                        $items = [];
                        foreach ($json as $item) {
