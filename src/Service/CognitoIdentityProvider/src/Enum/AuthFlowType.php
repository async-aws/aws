 ...
 * - `USER_SRP_AUTH` will take in `USERNAME` and `SRP_A` and return the SRP variables to be used for next challenge
 *   execution.
 ...
 * - `USER_PASSWORD_AUTH`: Non-SRP authentication flow; USERNAME and PASSWORD are passed directly. If a user migration
 *   Lambda trigger is set, this flow will invoke the user migration Lambda if the USERNAME is not found in the user
 *   pool.
 * - `ADMIN_USER_PASSWORD_AUTH`: Admin-based user password authentication. This replaces the `ADMIN_NO_SRP_AUTH`
 *   authentication flow. In this flow, Cognito receives the password in the request instead of using the SRP process to
 *   verify passwords.
 */
